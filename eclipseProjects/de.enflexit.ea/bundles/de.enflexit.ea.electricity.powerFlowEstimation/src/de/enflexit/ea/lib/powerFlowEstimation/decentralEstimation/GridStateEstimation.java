...
public class GridStateEstimation extends AbstractGridStateEstimation {
	...
	public boolean estimateGridState() {
		...
		// --- Checking, if new Method for Power Replacement is possible or old Linear
		...
		if (this.getDistrictAgentModel().isNewMethodPossible() == true) {
			...
			// --- This method checks, which loadAssignment (linear, bestCase, worstCase,
			// weighted) is most probably
		...
		HashMap<String, Double> pHashMap = this.createHashMapFromLoadPerNode(this.vLoadPerNode, this.vGenerationPerNode,"P");
		HashMap<String, Double> qHashMap = this.createHashMapFromLoadPerNode(this.vLoadPerNode, this.vGenerationPerNode,"Q");
		...
		this.estimationSuccessful = isEstimationSuccessful;
	...
	private HashMap<String, Double> setPVNodes() {
		...
		if (this.getDistrictAgentModel().getnNumSensors() > 1) {
			...
			Set<String> key = currentMeasurement.keySet();
			ArrayList<String> keySet = new ArrayList<>(key);

			for (int nIndexSensor = 0; nIndexSensor < keySet.size(); nIndexSensor++) {

				String nSensorName = keySet.get(nIndexSensor);

				double voltage = 0;

				NetworkComponent sensorComponent = this.getDistrictAgentModel().getNetworkModel().getNetworkComponent(nSensorName);
				Object[] dataModel = (Object[]) sensorComponent.getDataModel();
				SensorProperties sensor = (SensorProperties) dataModel[0];
				String networkID = sensor.getMeasureLocation();
...
//					String networkID = this.getDistrictAgentModel().getNodeNumberToNetworkComponentId().get(nNode);
	 ...
	 * @return
	 */
	private PowerFlowParameter preparingPFC(PowerFlowParameter powerFlowParameter, Vector<PowerPerNode> vLoadPerNode,
			Vector<PowerPerNode> vGenerationPerNode, int nNumNodes, boolean deltaVoltageCalculation) {
		...
		for (int i = 0; i < vGenerationPerNode.size(); i++) {
			...
			generationReal.set(index, pK.get(vGenerationPerNode.get(i).getnNode() - 1)
					+ vGenerationPerNode.get(i).getComplexPowerPerPhase().getReal());
			generationImag.set(index, qK.get(vGenerationPerNode.get(i).getnNode() - 1)
					+ vGenerationPerNode.get(i).getComplexPowerPerPhase().getImag());
		}
		...
		if (deltaVoltageCalculation == false) {
			...
			for (int a = 0; a < this.getDistrictAgentModel().getvSensorNames().size(); a++) {
				...
				if (this.getActualPhase().equals(Phase.L1) == true) {
					p = 1 * currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
							.getElectricalNodeState().getL1().getP().getValue();
					q = 1 * currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
							.getElectricalNodeState().getL1().getQ().getValue();
				}
				if (this.getActualPhase().equals(Phase.L2) == true) {
					p = 1 * currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
							.getElectricalNodeState().getL2().getP().getValue();
					q = 1 * currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
							.getElectricalNodeState().getL2().getQ().getValue();
				}
				if (this.getActualPhase().equals(Phase.L3) == true) {
					p = 1 * currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
							.getElectricalNodeState().getL3().getP().getValue();
					q = 1 * currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
							.getElectricalNodeState().getL3().getQ().getValue();
				}
				...
				if (nNodeIndex < 0) {
					...
					if (this.getActualPhase().equals(Phase.L1) == true) {
						voltage = currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
								.getElectricalNodeState().getL1().getVoltageAbs().getValue();
						current = currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
								.getElectricalNodeState().getL1().getCurrent().getValue();
					}
					if (this.getActualPhase().equals(Phase.L2) == true) {
						voltage = currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
								.getElectricalNodeState().getL2().getVoltageAbs().getValue();
						current = currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
								.getElectricalNodeState().getL2().getCurrent().getValue();
					}
					if (this.getActualPhase().equals(Phase.L3) == true) {
						voltage = currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
								.getElectricalNodeState().getL3().getVoltageAbs().getValue();
						current = currentMeasurement.get(this.getDistrictAgentModel().getvSensorNames().get(a))
								.getElectricalNodeState().getL3().getCurrent().getValue();
					}
	...
	private HashMap<String, Double> createHashMapFromLoadPerNode(Vector<PowerPerNode> loadPerNode,
			Vector<PowerPerNode> injectionPerNode, String kindOfPower) {
		HashMap<String, Double> hashMap = new HashMap<String, Double>();

		for (int i = 0; i < loadPerNode.size(); i++) {
			String networkComponent = this.getDistrictAgentModel().getNodeNumberToNetworkComponentId()
					.get(loadPerNode.get(i).getnNode());
		...
		for (int i = 0; i < injectionPerNode.size(); i++) {
			String networkComponent = this.getDistrictAgentModel().getNodeNumberToNetworkComponentId()
					.get(injectionPerNode.get(i).getnNode());
			if (kindOfPower.equals("P")) {
				hashMap.put(networkComponent, -1 * injectionPerNode.get(i).getComplexPowerPerPhase().getReal());
			} else {
				hashMap.put(networkComponent, injectionPerNode.get(i).getComplexPowerPerPhase().getImag());
			}
		}
	...
	private void calculateMeasuredDeltaVoltage() {
		...
		if (nFirstSensorName != null) {
			...
			if (this.getActualPhase().equals(Phase.L1) == true)
				voltageSensor2 = currentMeasurement.get(nFirstSensorName).getElectricalNodeState().getL1()
						.getVoltageAbs().getValue();
			if (this.getActualPhase().equals(Phase.L2) == true)
				voltageSensor2 = currentMeasurement.get(nFirstSensorName).getElectricalNodeState().getL2()
						.getVoltageAbs().getValue();
			if (this.getActualPhase().equals(Phase.L3) == true)
				voltageSensor2 = currentMeasurement.get(nFirstSensorName).getElectricalNodeState().getL3()
						.getVoltageAbs().getValue();
		}
		...
		if (nSecondSensorName != null) {
			...
			if (this.getActualPhase().equals(Phase.L1) == true)
				voltageSensor2 = currentMeasurement.get(nSecondSensorName).getElectricalNodeState().getL1()
						.getVoltageAbs().getValue();
			if (this.getActualPhase().equals(Phase.L2) == true)
				voltageSensor2 = currentMeasurement.get(nSecondSensorName).getElectricalNodeState().getL2()
						.getVoltageAbs().getValue();
			if (this.getActualPhase().equals(Phase.L3) == true)
				voltageSensor2 = currentMeasurement.get(nSecondSensorName).getElectricalNodeState().getL3()
						.getVoltageAbs().getValue();
	...
	private void calculateDERPowerOfAutarkicGrid() {
		...
		for (int i = 0; i < this.getDistrictAgentModel().getvNodeAssociation().size(); i++) {
			...
			// Trafo is not a DER
			if (this.getDistrictAgentModel().getvNodeAssociation().get(i).getnNode() != this.getDistrictAgentModel()
					.getnSlackNode()) {
				// KW-->1000W
...
//		selectedAssignment = 0;
	...
	private double calculateDeltaVoltage(Vector<PowerPerNode> vLoadPerNodePhase) {
		...
		if (this.getActualPhase().equals(Phase.L1) == true)
			dSlackVoltage = currentMeasurement.get(this.getDistrictAgentModel().getFirstSensor())
					.getElectricalNodeState().getL1().getVoltageAbs().getValue();
		if (this.getActualPhase().equals(Phase.L2) == true)
			dSlackVoltage = currentMeasurement.get(this.getDistrictAgentModel().getFirstSensor())
					.getElectricalNodeState().getL2().getVoltageAbs().getValue();
		...
		// --- Prepare PFC
		actualPowerFlowParameter = this.preparingPFC(actualPowerFlowParameter, this.vLoadPerNode,
				this.vGenerationPerNode, this.getDistrictAgentModel().getnNumNodes(), true);
		...
		int nIndexOfSlackNode = this.getDistrictAgentModel().getvBranchMeasurementParams()
				.get(this.getDistrictAgentModel().getnIndexFirstSensor()).getnFromNode() - 1;
		int nIndexOfSensor2 = this.getDistrictAgentModel().getvBranchMeasurementParams()
				.get(this.getDistrictAgentModel().getnIndexSecondSensor()).getnFromNode() - 1;
	...
	private HashMap<String, Double> integrateUnmeasuredNodes(HashMap<String, Double> hasmapInput) {
		...
		for (int i = 0; i < this.getDistrictAgentModel().getvNetworkComponents().size(); i++) {
			if (hasmapInput.get(this.getDistrictAgentModel().getvNetworkComponents().get(i).getId()) == null
					&& this.getDistrictAgentModel().getvNetworkComponents().get(i).getType().equals("Sensor") == false
					&& this.getDistrictAgentModel().getvNetworkComponents().get(i).getType().equals("Cable") == false) {
	...
	private Vector<PowerPerNode> integrateMeasuredNode(Vector<PowerPerNode> vecLoadPerNode) {
		...
		for (int nSensor = 0; nSensor < keySet.size(); nSensor++) {
			String sensorName = keySet.get(nSensor);
			NetworkComponent sensorComp = this.getDistrictAgentModel().getNetworkModel()
					.getNetworkComponent(sensorName);
			Vector<NetworkComponent> neighbouredComponent = this.getDistrictAgentModel().getNetworkModel()
					.getNeighbourNetworkComponents(sensorComp);
			Vector<NetworkComponent> neighbouredNodes = new Vector<>();

			// ---Filtering, only nodes
			for (int d = 0; d < neighbouredComponent.size(); d++) {
				if (neighbouredComponent.get(d).getType().equals("Sensor") == false
						&& neighbouredComponent.get(d).getType().equals("Cable") == false)
					neighbouredNodes.add(neighbouredComponent.get(d));
			}
			int node1 = this.getDistrictAgentModel().getNetworkComponentIdToNodeNumber()
					.get(neighbouredNodes.get(0).getId());
			int node2 = this.getDistrictAgentModel().getNetworkComponentIdToNodeNumber()
					.get(neighbouredNodes.get(1).getId());
			...
			for (int a = 0; a < this.getDistrictAgentModel().getvNodeAssociation().size(); a++) {
				...
				if (this.getDistrictAgentModel().getvNodeAssociation().get(a).getnNode() == measuredNode.getnNode()) {
					...
					if (this.getDistrictAgentModel().getvNodeAssociation().get(a).isLoadNode() == true
							|| this.getDistrictAgentModel().getvNodeAssociation().get(a).getdPowerAbs() != 0) {
	...
	private boolean buildingPowerSaldos() {
		...
		for (int nSensor = 0; nSensor < keySet.size(); nSensor++) {
			...
			// --- Influence
			dInfluence = this.getDistrictAgentModel().getvBranchMeasurementParams().get(nSensor)
					.getdInfluenceOnAutarkicGrid();
		...
		if (dPSaldoWithoutDER > 0) {
			...
			// System.err.println("Estimation Error: : Saldo of "+this.getActualPhase()+" is
			// negativ!");
	 ...
	 * 
	 * @see
	 * net.agenthygrid.estimation.decentralEstimation.AbstractGridStateEstimation#
